...
type ProviderSource =
  | { readonly kind: "browser"; readonly id: ProviderId; readonly name: string; readonly title: string; readonly description: string }
  | { readonly kind: "freesound"; readonly name: string; readonly title: string; readonly description: string };
...
export const PROVIDER_SOURCES = [
  ...
  },
  {
    kind: "freesound",
    name: "FREESOUND",
    title: "API Search and Preview",
    description: "Search Creative Commons sounds with your personal API key.",
  },
] as const satisfies readonly {
  readonly kind: ProviderSource["kind"];
...
type ProviderBrowserProps = {
  ...
  readonly onOpenFreesound: () => void;
  readonly status: string | null;
  readonly viewportRef: RefCallback<HTMLDivElement>;
};

export function ProviderBrowser({ activeProvider, mode, onBrowse, onOpenFreesound, status, viewportRef }: ProviderBrowserProps) {
  const isEmbeddedBrowserVisible = mode === "embedded" && activeProvider !== null;

  if (isEmbeddedBrowserVisible) {
    const source = PROVIDER_SOURCES.find((provider) => provider.kind === "browser" && provider.id === activeProvider);
    return (
      ...
      <div style={{ maxWidth: "760px", width: "100%" }}>
        ...
        <div style={{ borderTop: "1px solid #1f2937" }}>
          {PROVIDER_SOURCES.map((provider) => (
            <button key={provider.name} type="button" onClick={() => { if (provider.kind === "browser") { onBrowse(provider.id); return; } onOpenFreesound(); }} style={{ alignItems: "center", background: provider.kind === "browser" && activeProvider === provider.id ? "#111827" : "transparent", border: "none", borderBottom: "1px solid #1f2937", borderLeft: provider.kind === "browser" && activeProvider === provider.id ? "2px solid #f97316" : "2px solid transparent", color: "#e2e8f0", cursor: "pointer", display: "flex", fontFamily: "'Courier New', monospace", gap: "16px", padding: "14px 12px", textAlign: "left", width: "100%" }}>
